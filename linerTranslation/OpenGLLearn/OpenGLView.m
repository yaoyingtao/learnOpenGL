...
- (void)render:(CADisplayLink*)displayLink {
    ...
    [projection scaleBy:CC3VectorMake(2.0, 1.0, 1.0)];
    [projection rotateByZ:180];
    [projection translateByY:0.5];

    glUniformMatrix4fv(_projectionUniform, 1, 0, projection.glMatrix);
    

    glVertexAttribPointer(_positionSlot, 3, GL_FLOAT, GL_FALSE, 8*sizeof(float), 0);
    ...
    CC3GLMatrix *projection1 = [[CC3GLMatrix alloc] initIdentity];
    [projection1 scaleBy:CC3VectorMake(1.5, 0.5, 1.0)];
    [projection1 translateByY:1];
    NSInteger time = [[NSDate date] timeIntervalSince1970];
    [projection1 rotateByZ:time%180];

    glUniformMatrix4fv(_projectionUniform, 1, 0, projection1.glMatrix);
    glDrawElements(GL_TRIANGLES, sizeof(indices)/sizeof(indices[0]), GL_UNSIGNED_BYTE, 0);
